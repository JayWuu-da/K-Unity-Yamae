...
class MiMoAgent(BaseAgent):
    def execute(
        ...
        api_key = os.environ.get(self.agent_config.get("api_key_env", "MIMO_API_KEY"), "")
        if not api_key:
            return {"status": "error", "message": "MIMO_API_KEY not set"}

        max_retries = self.agent_config.get("max_retries", 3)
        endpoint = self.agent_config.get("endpoint", "https://api.mimo.ai/v1/chat/completions")
        timeout = self.agent_config.get("timeout", 120)

        for attempt in range(max_retries):
            try:
                prompt = self._build_prompt(task, risk_report, mode, project_path)
                model = self.agent_config.get("model", "mimo-auto")
                payload = json.dumps(
                    {
                        "model": model,
                        "messages": [
                ...
                ledger.add_event(
                    "agent_output",
                    {"agent": "mimo", "preview": result_text[:500], "changes": len(changes)},
                )
