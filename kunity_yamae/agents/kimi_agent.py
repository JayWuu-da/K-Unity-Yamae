...
class KimiAgent(BaseAgent):
    def execute(
        ...
        api_key = os.environ.get(self.agent_config.get("api_key_env", "KIMI_API_KEY"), "")
        if not api_key:
            return {"status": "error", "message": "KIMI_API_KEY not set"}

        max_retries = self.agent_config.get("max_retries", 3)
        endpoint = self.agent_config.get("endpoint", "https://api.moonshot.cn/v1/chat/completions")
        timeout = self.agent_config.get("timeout", 120)

        for attempt in range(max_retries):
            try:
                prompt = self._build_prompt(task, risk_report, mode, project_path)
                model = self.agent_config.get("model", "moonshot-v1-128k")
                payload = json.dumps(
                    {
                        "model": model,
                        "messages": [
                ...
                ledger.add_event(
                    "agent_output",
                    {"agent": "kimi", "preview": result_text[:500], "changes": len(changes)},
                )
